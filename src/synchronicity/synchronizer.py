...
Traceback:{self._thread_traceback}"""
    ...
    def register_external_wrapper_class(self, impl_cls, wrapper_cls):
        """Register a wrapper class generated by another synchronizer."""
        interfaces = impl_cls.__dict__.get(self._wrapped_attr)
        if interfaces is None:
            interfaces = {}
            setattr(impl_cls, self._wrapped_attr, interfaces)
        existing = interfaces.get(Interface.BLOCKING)
        if existing is not None and existing is not wrapper_cls:
            raise RuntimeError(f"{impl_cls} already has blocking wrapper {existing}")
        interfaces[Interface.BLOCKING] = wrapper_cls

    def register_external_wrapper_instance(self, impl_instance, wrapper_instance):
        """Associate an external wrapper instance with its implementation instance."""
        interfaces = impl_instance.__dict__.setdefault(self._wrapped_attr, {})
        existing = interfaces.get(Interface.BLOCKING)
        if existing is not None and existing is not wrapper_instance:
            raise RuntimeError(f"{impl_instance} already has blocking wrapper {existing}")
        interfaces[Interface.BLOCKING] = wrapper_instance
        wrapper_instance.__dict__[self._original_attr] = impl_instance
        wrapper_instance.__dict__[SYNCHRONIZER_ATTR] = self
