...
class TypeTransformer(ABC):
    ...
    @abstractmethod
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
    ...
    @abstractmethod
    def unwrap_expr(self, synchronizer: "Synchronizer", var_name: str) -> str:
    ...
    @abstractmethod
    def wrap_expr(self, synchronizer: "Synchronizer", target_module: str, var_name: str) -> str:
...
class WrappedClassTransformer(TypeTransformer):
    ...
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
        ...
        if self.impl_type not in synchronizer._wrapped:
            # Should not happen if create_transformer is used correctly
            return _format_annotation_str(self.impl_type)

        wrapper_target_module, wrapper_name = synchronizer._wrapped[self.impl_type]
...
class ListTransformer(TypeTransformer):
    ...
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
        ...
        item_type_str = self.item_transformer.wrapped_type(synchronizer, target_module)
    ...
    def unwrap_expr(self, synchronizer: "Synchronizer", var_name: str) -> str:
        ...
        item_unwrap = self.item_transformer.unwrap_expr(synchronizer, "x")
    ...
    def wrap_expr(self, synchronizer: "Synchronizer", target_module: str, var_name: str) -> str:
        ...
        item_wrap = self.item_transformer.wrap_expr(synchronizer, target_module, "x")
...
class DictTransformer(TypeTransformer):
    ...
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
        ...
        key_type_str = self.key_transformer.wrapped_type(synchronizer, target_module)
        value_type_str = self.value_transformer.wrapped_type(synchronizer, target_module)
    ...
    def unwrap_expr(self, synchronizer: "Synchronizer", var_name: str) -> str:
        ...
        value_unwrap = self.value_transformer.unwrap_expr(synchronizer, "v")
    ...
    def wrap_expr(self, synchronizer: "Synchronizer", target_module: str, var_name: str) -> str:
        ...
        value_wrap = self.value_transformer.wrap_expr(synchronizer, target_module, "v")
...
class TupleTransformer(TypeTransformer):
    ...
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
        ...
        if len(self.item_transformers) == 1:
            ...
            item_type_str = self.item_transformers[0].wrapped_type(synchronizer, target_module)
            return f"tuple[{item_type_str}, ...]"
        else:
            # Fixed-size tuple: tuple[T1, T2, ...]
            item_type_strs = [t.wrapped_type(synchronizer, target_module) for t in self.item_transformers]
    ...
    def unwrap_expr(self, synchronizer: "Synchronizer", var_name: str) -> str:
        ...
        if len(self.item_transformers) == 1:
            ...
            item_unwrap = self.item_transformers[0].unwrap_expr(synchronizer, "x")
            return f"tuple({item_unwrap} for x in {var_name})"
        else:
            # Fixed-size tuple - unwrap each element by index
            unwrap_exprs = [
                t.unwrap_expr(synchronizer, f"{var_name}[{i}]") for i, t in enumerate(self.item_transformers)
            ]
    ...
    def wrap_expr(self, synchronizer: "Synchronizer", target_module: str, var_name: str) -> str:
        ...
        if len(self.item_transformers) == 1:
            # Variable-length tuple
            item_wrap = self.item_transformers[0].wrap_expr(synchronizer, target_module, "x")
            return f"tuple({item_wrap} for x in {var_name})"
        else:
            # Fixed-size tuple - wrap each element by index
            wrap_exprs = [
                t.wrap_expr(synchronizer, target_module, f"{var_name}[{i}]")
...
class OptionalTransformer(TypeTransformer):
    ...
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
        ...
        inner_type_str = self.inner_transformer.wrapped_type(synchronizer, target_module)
    ...
    def unwrap_expr(self, synchronizer: "Synchronizer", var_name: str) -> str:
        ...
        inner_unwrap = self.inner_transformer.unwrap_expr(synchronizer, var_name)
    ...
    def wrap_expr(self, synchronizer: "Synchronizer", target_module: str, var_name: str) -> str:
        ...
        inner_wrap = self.inner_transformer.wrap_expr(synchronizer, target_module, var_name)
...
class GeneratorTransformer(TypeTransformer):
    ...
    def wrapped_type(self, synchronizer: "Synchronizer", target_module: str) -> str:
        ...
        yield_type_str = self.yield_transformer.wrapped_type(synchronizer, target_module)
    ...
    def get_yield_wrap_expr(self, synchronizer: "Synchronizer", target_module: str, var_name: str) -> str:
        """Get the wrap expression for each yielded item."""
        return self.yield_transformer.wrap_expr(synchronizer, target_module, var_name)


def create_transformer(annotation, synchronizer: "Synchronizer") -> TypeTransformer:
    ...
    if origin is list:
        if args:
            item_transformer = create_transformer(args[0], synchronizer)
    ...
    if origin is dict:
        if len(args) >= 2:
            key_transformer = create_transformer(args[0], synchronizer)
            value_transformer = create_transformer(args[1], synchronizer)
    ...
    if origin is tuple:
        if args:
            ...
            if Ellipsis in args:
                ...
                item_transformers = [create_transformer(arg, synchronizer) for arg in args]
    ...
    if origin is typing.Union:
        ...
        if len(non_none_args) == 1 and type(None) in args:
            inner_transformer = create_transformer(non_none_args[0], synchronizer)
    ...
    if origin is collections.abc.Generator or origin is collections.abc.Iterator:
        if args:
            yield_transformer = create_transformer(args[0], synchronizer)
