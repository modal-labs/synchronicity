"""Tests for type translation utilities in synchronicity2.compile."""

import pytest
import typing

from synchronicity2.compile import (
    _build_unwrap_expr,
    _build_wrap_expr,
    _get_wrapped_classes,
    _needs_translation,
    _translate_type_annotation,
)
...
class TestGetWrappedClasses:
    ...
    def test_ignores_non_classes(self):
        ...
        result = _get_wrapped_classes(items_with_function)
        assert "Foo" in result
        assert "some_function" not in result


class TestTranslateTypeAnnotation:
    """Tests for _translate_type_annotation()."""

    def test_direct_wrapped_type(self, wrapped_classes):
        """Test translating a direct wrapped class type."""
        wrapper_str, impl_str = _translate_type_annotation(_impl_Foo, wrapped_classes, "test_translation_utils_test")
    ...
    def test_list_of_wrapped_type(self, wrapped_classes):
        ...
        wrapper_str, impl_str = _translate_type_annotation(annotation, wrapped_classes, "test_translation_utils_test")
    ...
    def test_primitive_type_unchanged(self, wrapped_classes):
        ...
        wrapper_str, impl_str = _translate_type_annotation(str, wrapped_classes, "test_translation_utils_test")
    ...
    def test_any_type_unchanged(self, wrapped_classes):
        ...
        wrapper_str, impl_str = _translate_type_annotation(typing.Any, wrapped_classes, "test_translation_utils_test")
        assert wrapper_str == impl_str
        assert "Any" in wrapper_str


class TestNeedsTranslation:
    """Tests for _needs_translation()."""

    def test_wrapped_type_needs_translation(self, wrapped_classes):
        """Test that wrapped types are detected."""
        assert _needs_translation(_impl_Foo, wrapped_classes) is True

    def test_list_of_wrapped_needs_translation(self, wrapped_classes):
        """Test that list[WrappedClass] needs translation."""
        annotation = typing.List[_impl_Bar]
        assert _needs_translation(annotation, wrapped_classes) is True

    def test_primitive_no_translation(self, wrapped_classes):
        """Test that primitives don't need translation."""
        assert _needs_translation(str, wrapped_classes) is False
        assert _needs_translation(int, wrapped_classes) is False

    def test_any_no_translation(self, wrapped_classes):
        """Test that typing.Any doesn't need translation."""
        assert _needs_translation(typing.Any, wrapped_classes) is False

    def test_empty_annotation_no_translation(self, wrapped_classes):
        """Test that Signature.empty doesn't need translation."""
        import inspect

        assert _needs_translation(inspect.Signature.empty, wrapped_classes) is False


class TestBuildUnwrapExpr:
    """Tests for _build_unwrap_expr()."""

    def test_direct_wrapped_type(self, wrapped_classes):
        """Test unwrapping a direct wrapped type."""
        expr = _build_unwrap_expr(_impl_Foo, wrapped_classes, "my_var")
        assert expr == "my_var._impl_instance"

    def test_list_of_wrapped(self, wrapped_classes):
        """Test unwrapping list[WrappedClass]."""
        annotation = typing.List[_impl_Foo]
        expr = _build_unwrap_expr(annotation, wrapped_classes, "items")
    ...
    def test_dict_with_wrapped_values(self, wrapped_classes):
        ...
        annotation = typing.Dict[str, _impl_Bar]
        expr = _build_unwrap_expr(annotation, wrapped_classes, "mapping")
    ...
    def test_optional_wrapped_type(self, wrapped_classes):
        ...
        annotation = typing.Optional[_impl_Foo]
        expr = _build_unwrap_expr(annotation, wrapped_classes, "maybe_val")
    ...
    def test_tuple_of_wrapped(self, wrapped_classes):
        ...
        expr = _build_unwrap_expr(annotation, wrapped_classes, "tup")
    ...
    def test_nested_list_dict(self, wrapped_classes):
        ...
        expr = _build_unwrap_expr(annotation, wrapped_classes, "data")
    ...
    def test_primitive_no_unwrap(self, wrapped_classes):
        ...
        expr = _build_unwrap_expr(str, wrapped_classes, "text")
        assert expr == "text"

    def test_any_no_unwrap(self, wrapped_classes):
        """Test that typing.Any is not unwrapped."""
        expr = _build_unwrap_expr(typing.Any, wrapped_classes, "anything")
        assert expr == "anything"


class TestBuildWrapExpr:
    """Tests for _build_wrap_expr()."""

    def test_direct_wrapped_type(self, wrapped_classes):
        """Test wrapping a direct wrapped type."""
        expr = _build_wrap_expr(_impl_Foo, wrapped_classes, "result")
        assert expr == "_wrap_Foo(result)"

    def test_list_of_wrapped(self, wrapped_classes):
        """Test wrapping list[WrappedClass]."""
        annotation = typing.List[_impl_Bar]
        expr = _build_wrap_expr(annotation, wrapped_classes, "items")
    ...
    def test_dict_with_wrapped_values(self, wrapped_classes):
        ...
        expr = _build_wrap_expr(annotation, wrapped_classes, "mapping")
    ...
    def test_optional_wrapped_type(self, wrapped_classes):
        ...
        expr = _build_wrap_expr(annotation, wrapped_classes, "maybe_val")
    ...
    def test_tuple_of_wrapped(self, wrapped_classes):
        ...
        annotation = typing.Tuple[_impl_Foo, ...]
        expr = _build_wrap_expr(annotation, wrapped_classes, "tup")
    ...
    def test_nested_list_dict(self, wrapped_classes):
        ...
        expr = _build_wrap_expr(annotation, wrapped_classes, "data")
        # Should have nested comprehensions
    ...
    def test_primitive_no_wrap(self, wrapped_classes):
        """Test that primitives are returned as-is."""
        expr = _build_wrap_expr(int, wrapped_classes, "number")
        assert expr == "number"

    def test_any_no_wrap(self, wrapped_classes):
        """Test that typing.Any is not wrapped."""
        expr = _build_wrap_expr(typing.Any, wrapped_classes, "anything")
        assert expr == "anything"
...
class TestEdgeCases:
    ...
    def test_multiple_wrapped_classes_in_dict(self, wrapped_classes):
        ...
        _needs_translation(annotation, wrapped_classes)
    ...
    def test_deeply_nested_structure(self, wrapped_classes):
        ...
        expr = _build_unwrap_expr(annotation, wrapped_classes, "deep")
    ...
    def test_empty_wrapped_classes(self):
        ...
        empty_wrapped = {}
        assert _needs_translation(_impl_Foo, empty_wrapped) is False
        expr = _build_unwrap_expr(_impl_Foo, empty_wrapped, "val")
